...
func TestAttachmentDownloadFromWebhook(t *testing.T) {
	...
	attachment := WebhookAttachment{
		...
		ContentID:   "test-id",
		DownloadUrl: server.URL + "/attachments/email-123/test.txt",
	}

	data, err := client.Attachment().Download(context.Background(), "email-123", attachment.Filename)
	if err != nil {
